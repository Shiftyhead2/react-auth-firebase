  ...
  return (
    <Breadcrumb className = "flex-grow-1" listProps = {{className: "bg-white pl-0 m-0"}}>
      {[...path].map((folder,index) => (
        <Breadcrumb.Item key = {folder.id} className = "text-truncate d-inline-block" style = {{maxWidth: '150px'}} linkAs = {Link} linkProps = {{to: { pathname:folder.id ? `/folder/${folder.id}`: "/" , state: {folder: {...folder, path: path.slice(1,index)}}}}}>{folder.name}</Breadcrumb.Item>
      ))}
      {currentFolder && (
